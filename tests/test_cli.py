...
@pytest.mark.parametrize('port, expectation', [
...
def test_argparser_error_msg(argparser):
    with pytest.raises(exceptions.ArgumentError,
                       match='^__main__.py: error message'):
        assert argparser.error('error message')


@pytest.mark.parametrize('namespace, expectation', [
    (argparse.Namespace(
        github_auth_type='app', github_app_id=None,
        github_installation_id=123, github_app_private_key_path='/path'),
        pytest.raises(exceptions.ArgumentError)
    ),
    (argparse.Namespace(
        github_auth_type='app', github_app_id=4455,
        github_app_installation_id=None, github_app_private_key_path='/path'),
        pytest.raises(exceptions.ArgumentError)
    ),
    (argparse.Namespace(
        github_auth_type='app', github_app_id=123,
        github_app_installation_id=123, github_app_private_key_path=None),
        pytest.raises(exceptions.ArgumentError)
    ),
    (argparse.Namespace(
        github_auth_type='pat', github_token=None),
        pytest.raises(exceptions.ArgumentError)
    ),
    (argparse.Namespace(
        github_auth_type='unknown'),
        pytest.raises(exceptions.ArgumentError)
    ),
    (argparse.Namespace(
        github_auth_type='pat', github_token='token'),
        does_not_raise()
    ),
    (argparse.Namespace(
        github_auth_type='app', github_app_id=123,
        github_app_installation_id=123, github_app_private_key_path='/path'),
        does_not_raise()
    )
])
def test_mutual_inclusive_args(namespace, argparser, expectation):
    with expectation:
        cli._check_mutual_inclusive_arguments(
            namespace,
            argparser
        )


@pytest.mark.parametrize('github_env_vars', [
    {
